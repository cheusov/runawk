...
libraries (modules). B<runawk> does this.
...
B<NOTE:> all options must be specified as a single token, i.e. I<-Vh>,
not I<-V -h>
...
Modules are first searched in a directory were main
program (module) is placed, then AWKPATH environment variable is
checked.  AWKPATH variable specifies a colon separated
list of search directories where modules are searched if the path
in I<#use> directive is relative.
Absolute path to the module is also allowed.
