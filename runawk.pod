...
AWK lacks support for modules. Even if you create small programs, you
need have library functions that are used regulary. So, you need
support modules.
...
arguments with -- (two minus signes). In my view, this is badly.
...
When C<#!/usr/bin/awk -f> script handles arguments (options) and needs
to read from stdin when nomfiles are specified, it is necessary to add
/dev/stdin or `-' as a file explicitely.
This is badly too.
